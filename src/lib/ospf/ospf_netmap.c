/* 
 * $Id: ospf_netmap.c,v 1.1.1.1 2000/08/14 18:46:12 labovit Exp $
 */
...
ospf_vertex_t *ospf_create_net_map (ospf_area_t *ospf_area) {
  ...
  trace (NORM, OSPF.trace, "Starting to create net map for area %d\n",
	 ospf_area->area_id);
  ...
  trace (NORM, OSPF.trace, "Number of links %d\n", root_router_lsa.num_links);
  ...
  while (curr_vertex != NULL) {
    ...
    if (curr_vertex->router_lsa != NULL) {
      ...
      trace (NORM, OSPF.trace, "Processing Router Vertex %s\n",
	...
	trace (NORM, OSPF.trace, "Found a link going out. %s\n",
	       long_inet_ntoa (curr_vertex->vertex_id));
	...
	if (!found) {
      ...
      trace (NORM, OSPF.trace, "Processing Network Vertex %s\n",
	     long_inet_ntoa (curr_vertex->vertex_id));
