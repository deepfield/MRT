/* 
 * $Id: lsa_database.c,v 1.1.1.1 2000/08/14 18:46:15 labovit Exp $
 */
...
void ospf_database_dump () {
  schedule_event2 ("ospf_databse_dump", OSPF.schedule, _ospf_database_dump, 0, NULL);
}
static void _ospf_database_dump () {
  char name[MAXLINE];
  FILE *fd;
  ...
  if ((fd = fopen (name, "w")) == NULL) {
    trace (ERROR, default_trace, "OSPF error -- could not open %s for dumping database\n",
	   name);
    return;
  }
  _show_ospf_database (NULL, fd);
  fclose (fd);

  trace (NORM, default_trace, "OSPF Dumped LSA database to %s\n", name);
  return;
}
...
void show_ospf_database (uii_connection_t *uii) {
  schedule_event2 ("_show_ospf_database", OSPF.schedule, _show_ospf_database, 2, 
		   uii, NULL);
}
static void _show_ospf_database (uii_connection_t *uii, FILE *fd) {
...
void ospf_add_lsa_to_db (ospf_lsa_t *header, void *lsa) {
  ...
  case OSPF_EXTERNAL_LSA:
    ...
    break;
  default:
    trace (ERROR, default_trace, "ERROR -- Attempting to add "
...
void *ospf_find_lsa_in_db (ospf_lsa_t *header) {
  ...
  case OSPF_SUMMARY_LSA4:
    ...
    break;
  default:
    trace (ERROR, default_trace, "ERROR -- Attempting to find "
